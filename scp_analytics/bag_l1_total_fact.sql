INSERT OVERWRITE TABLE bag_l1_total_fact
select 
distinct
...
X.bag_id,
X.bag_source_hub_id,
X.bag_assigned_hub_id,
X.preprimary_fac,
X.assigned_hub_id,
X.ekl_shipment_type,
X.rto_create_datetime,
X.bag_first_closed_datetime,
X.rto_bag_flag,
X.shipment_created_at_datetime,
X.shipment_delivered_at_datetime,
X.shipment_first_received_at_datetime,
DENSE_RANK() OVER (PARTITION BY X.vendor_tracking_id,X.rto_bag_flag ORDER BY X.bag_id ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as rank,
lookupkey('facility_id',X.origin_mh_id) as origin_mh_id_key,
lookupkey('facility_id',X.assigned_hub_id) as assigned_hub_id_key,
lookupkey('facility_id',X.bag_source_hub_id) as bag_source_hub_id_key,
lookupkey('facility_id',X.bag_assigned_hub_id) as bag_assigned_hub_id_key,
lookup_date(X.shipment_created_at_datetime) as shipment_created_at_date_key,
lookup_time(X.shipment_created_at_datetime) as shipment_created_at_time_key,
lookup_date(X.shipment_first_received_at_datetime) as shipment_first_received_at_date_key,
lookup_time(X.shipment_first_received_at_datetime) as shipment_first_received_at_time_key,
lookup_date(X.shipment_delivered_at_datetime) as shipment_delivered_at_date_key,
lookup_time(X.shipment_delivered_at_datetime) as shipment_delivered_at_time_key,
lookup_date(X.bag_first_closed_datetime) as bag_first_closed_date_key,
lookup_time(X.bag_first_closed_datetime) as bag_first_closed_time_key
from
(select 
A.vendor_tracking_id as vendor_tracking_id,
A.shipment_origin_mh_facility_id as origin_mh_id,
C.bag_id as bag_id,
C.bag_source_hub_id as bag_source_hub_id,
C.bag_assigned_hub_id as bag_assigned_hub_id,
case when D.preprimary_fac is null then 0 else D.preprimary_fac end as preprimary_fac,
A.fsd_assigned_hub_id as assigned_hub_id,
ekl_shipment_type,
rto_create_datetime,
bag_first_closed_datetime,
case when rto_create_datetime < bag_first_closed_datetime then 1 else 0 end as rto_bag_flag,
A.shipment_created_at_datetime,
A.shipment_delivered_at_datetime,
A.shipment_first_received_at_datetime
from
bigfoot_external_neo.scp_ekl__shipment_l1_90_fact A
left join
bigfoot_external_neo.scp_ekl__shipment_closedbag_map_l1_90_fact B on A.vendor_tracking_id = B.shipment_id
left join
bigfoot_external_neo.scp_ekl__bag_l1_90_fact C on cast(split(B.bag,"-")[1] as INT) = C.bag_id
left join
(select data.trackingid as trackingid, 1 as preprimary_flag, data.sortfacilityid as preprimary_fac from bigfoot_journal.dart_wsr_scp_ekl_hubsystemsortationdetail_3 where data.processingareatype = 'CONSOLIDATED_STATION' and day >= 20161228) D on D.trackingid = A.vendor_tracking_id
where A.shipment_carrier = 'FSD' and A.shipment_dispatch_datetime is not null and C.bag_created_hub_type <> 'DELIVERY_HUB')X
