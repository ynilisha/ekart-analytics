INSERT OVERWRITE TABLE sbr_tc_breach_60_fact
select
B.vendor_tracking_id as vendor_tracking_id,
If(B.processing_cue_flag = 1,
  If (bag_history.bag_closed_datetime is  null,
    0,
    If(unix_timestamp(bag_history.bag_closed_datetime) - unix_timestamp(B.mh_received_at_datetime) >=90000,
      1,
      0
      )
    ),
  ...
  If (B.processing_cue_flag = 1,
    IF(B.incoming_consignment_create_unixseconds is null,
      0,
      If( lead_time > 3600 and (B.incoming_consignment_create_unixseconds - unix_timestamp(B.mh_received_at_datetime) >= lead_time + 10800) ,
        1,
        IF(lead_time < 3600 and (B.incoming_consignment_create_unixseconds - unix_timestamp(B.mh_received_at_datetime) >=lead_time + 86400+10800),1,0)
        )
      ),
  IF(B.incoming_consignment_create_unixseconds is null,
    0,
...
FROM(
  ...
  FROM(
    ...
    if (S.incoming_consignment_source_hub_id = S.shipment_origin_mh_facility_id,
      ...
      S.shipment_facility_inscan_datetime
  ...
  (select source_id,
    ...
    processing_cue_flag
  FROM
  bigfoot_external_neo.scp_ekl__service_based_routing_ideal_connection_60_fact) ICF
  on 
